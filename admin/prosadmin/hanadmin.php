...
if (empty($_GET['thisposition'])){
	...
	require 'rootadmin.php'; require '../../config/koneksi.php';
	$ru = new con();
	$to=base64_decode($_GET['thisposition']);
	if($to=='loginadmin') {
		$ru->loginadmin($con, $_POST['user'], $_POST['pass']);
	}
	elseif($to=='logoutadmin'){
		$ru->logoutadmin();
	}
	elseif ($to=='kebutuhan') {
		$air = $_POST['air'];
	...
	elseif($to=='addkaryawan'){
		$ru->addkaryawan($con,$_POST['user'],md5($_POST['password']),$_POST['nama'],$_POST['alamat']);
	}
	else if($to=='deletekaryawan'){ $control=base64_decode($_GET['thiscontrol']);
		$ru->deletekaryawan($con,$control);
	}
}
?>